<?php
/**
 * paypal_create_card_order.php - Create PayPal Order for Advanced Card Fields
...
function failCard($code, $msg) {
  global $rid;
  error_log("RID={$rid} CARD_ORDER RESP={$code} MSG={$msg}");
  http_response_code($code);
  jsonOutCard(array('success' => false, 'message' => $msg, 'rid' => $rid));
}

$method = isset($_SERVER['REQUEST_METHOD']) ? $_SERVER['REQUEST_METHOD'] : '';
if ($method === 'OPTIONS') { http_response_code(204); exit; }
if ($method !== 'POST') failCard(405, 'Method not allowed');

// Auth
$user = getCurrentUser();
if (!$user) failCard(401, 'Not authenticated');
$userId = (int)$user['user_id'];

// Parse input
$payload = getJsonInput();
$items = isset($payload['items']) && is_array($payload['items']) ? $payload['items'] : array();
if (count($items) < 1) failCard(400, 'Cart items required');
...
if ($isGift) {
  if ($giftCharacterName === '') failCard(400, 'Please enter the recipient character name');
  if (strlen($giftCharacterName) > 50) failCard(400, 'Character name too long');
  ...
  try {
    ...
    if (!$recipient) {
      failCard(400, 'Character "' . $giftCharacterName . '" not found. Please check the name.');
    }
    ...
    failCard(500, 'Failed to look up character');
  }
} else {
  if ($characterId <= 0) failCard(400, 'Please select a character to receive the items');
}

// PayPal config
$ppCfg = getPayPalConfig();
if ($ppCfg['client_id'] === '' || $ppCfg['secret'] === '') {
  failCard(500, 'Payment not configured');
}
...
foreach ($items as $it) {
  ...
  if ($qty < 1) $qty = 1;
  if ($qty > 99) $qty = 99;
  if ($price <= 0 || $price < 0.50) continue;
...
if (count($ppItems) < 1) failCard(400, 'No valid items');

$totalFormatted = number_format($totalAmount, 2, '.', '');

// Purchase units - no redirect URLs needed for card fields
$purchaseUnits = array(
  array(
...
// Encode metadata into custom_id
...
if ($tokenResult['error'] !== '') {
  error_log("RID={$rid} CARD_TOKEN_ERR: " . $tokenResult['error']);
  failCard(502, 'Payment provider error');
}

// Create order via PayPal API directly (no redirect URLs)
$baseUrl = getPayPalBaseUrl($ppCfg['sandbox']);
...
list($code, $data, $err) = paypalRequest('POST', $url, $tokenResult['token'], $orderData);
...
if ($err !== '' || $code < 200 || $code >= 300) {
  ...
  error_log("RID={$rid} CARD_ORDER_ERR: " . $errMsg);
  failCard(502, 'Payment provider error');
}

$paypalOrderId = isset($data['id']) ? $data['id'] : '';
if ($paypalOrderId === '') {
  failCard(502, 'Failed to create payment order');
}
...
try {
  ...
  error_log("RID={$rid} CARD_ORDER_CREATED paypal_order={$paypalOrderId} user={$userId} orders={$ordersCreated}");
} catch (Exception $e) {
  error_log("RID={$rid} CARD_ORDER_DB_ERR: " . $e->getMessage());
}

jsonOutCard(array('success' => true, 'orderID' => $paypalOrderId, 'rid' => $rid));
