...
export const achievementsApi = {
  async listAchievements(): Promise<Achievement[]> {
    const response = await fetch(`${API_BASE}/achievements.php?action=list`, {
      cache: "no-store",
  ...
  async getUserProgress(): Promise<{ achievements: UserAchievement[]; stats: AchievementStats }> {
    const response = await fetch(`${API_BASE}/achievements.php?action=user_progress`, {
      credentials: "include",
  ...
  async checkUnlocks(): Promise<{ newlyUnlocked: NewlyUnlocked[]; count: number }> {
    const response = await fetch(`${API_BASE}/achievements.php?action=check_unlocks`, {
      credentials: "include",
      headers: { ...getAuthHeaders(), Accept: "application/json" },
    });
    const data = await response.json();
    return { newlyUnlocked: data.newly_unlocked || [], count: data.count || 0 };
  },

  async claimReward(achievementId: number): Promise<{ success: boolean; coins: number; vip: number; error?: string }> {
    const response = await fetch(`${API_BASE}/achievements.php?action=claim`, {
      method: "POST",
      credentials: "include",
    ...
    const data = await response.json();
    return { success: data.success === true, coins: data.coins_earned || 0, vip: data.vip_earned || 0, error: data.error };
  },

  async adminListAll(): Promise<Achievement[]> {
    const response = await fetch(`${API_BASE}/achievements.php?action=list_all`, {
      credentials: "include",
      headers: { ...getAuthHeaders(), Accept: "application/json" },
    });
    const data = await response.json();
  ...
  async adminAdd(achievement: Partial<Achievement>): Promise<{ success: boolean; id?: number; error?: string }> {
    const response = await fetch(`${API_BASE}/achievements.php?action=add`, {
      method: "POST",
      credentials: "include",
      headers: { "Content-Type": "application/json", ...getAuthHeaders() },
  ...
  async adminUpdate(id: number, updates: Partial<Achievement>): Promise<boolean> {
    const response = await fetch(`${API_BASE}/achievements.php?action=update`, {
      method: "POST",
      credentials: "include",
      headers: { "Content-Type": "application/json", ...getAuthHeaders() },
  ...
  async adminDelete(id: number): Promise<boolean> {
    const response = await fetch(`${API_BASE}/achievements.php?action=delete`, {
      method: "POST",
      credentials: "include",
      headers: { "Content-Type": "application/json", ...getAuthHeaders() },
  ...
  async adminGetStats(): Promise<{
    ...
    const response = await fetch(`${API_BASE}/achievements.php?action=stats`, {
      credentials: "include",
      headers: { ...getAuthHeaders(), Accept: "application/json" },
    });
    const data = await response.json();
